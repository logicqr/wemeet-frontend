...
function MyMeeting() {
    ...
    const fetchMeetings = async () => {
        ...
        try {
            ...
            const response = await axios.post("https://wemeet-backend-latest.onrender.com/api/my-meetings", { user_id });
    ...
    const todaysFutureMeetings = meetings.filter(meeting => 
        isToday(meeting.scheduledAt) && !isPastMeeting(meeting.scheduledAt)
    ).sort((a, b) => new Date(a.scheduledAt) - new Date(b.scheduledAt));
    
    const todaysPastMeetings = meetings.filter(meeting => 
        isToday(meeting.scheduledAt) && isPastMeeting(meeting.scheduledAt)
    ...
    const renderMeetingCard = (meeting) => {
        ...
        const now = new Date();
        const meetingDate = new Date(meeting.scheduledAt);
        const canJoinMeeting = isToday(meeting.scheduledAt) && !isPastMeeting(meeting.scheduledAt);
        ...
        return (
            ...
            >
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                    ...
                    {meeting.user && (
                        <div className="flex items-center text-gray-600">
                            ...
                            <span>{meeting.user.userName} ({meeting.user.position})</span>
                        </div>
                    )}
                </div>
                ...
                {meeting.description && (
                    <p className="mt-3 text-gray-600 text-sm line-clamp-2">{meeting.description}</p>
                )}
    ...
    const renderMeetingsList = (meetingsList, emptyMessage) => {
        ...
        return (
            <div className="space-y-4">
                {meetingsList.map(renderMeetingCard)}
            </div>
        );
    };
    ...
    const renderFilterTabs = () => (
        ...
        <button 
          ...
          className={`px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-200 shadow-sm flex items-center gap-2 ${ 
            activeView === 'all' ? 'bg-blue-600 text-white shadow-blue-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' 
          }`}
        >
        ...
        <button 
          ...
          className={`px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-200 shadow-sm flex items-center gap-2 ${ 
            activeView === 'today' ? 'bg-blue-600 text-white shadow-blue-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' 
          }`}
        >
        ...
        <button 
          ...
          className={`px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-200 shadow-sm flex items-center gap-2 ${ 
            activeView === 'upcoming' ? 'bg-blue-600 text-white shadow-blue-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' 
          }`}
        >
        ...
        <button 
          ...
          className={`px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-200 shadow-sm flex items-center gap-2 ${ 
            activeView === 'past' ? 'bg-blue-600 text-white shadow-blue-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' 
          }`}
        >
            ...
            return (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="border-t-4 border-blue-500 rounded-lg bg-white p-4 shadow">
                        ...
                        {renderMeetingsList(todaysMeetings, "No meetings scheduled for today.")}
                    </div>
                    ...
                    <div className="border-t-4 border-green-500 rounded-lg bg-white p-4 shadow">
                        ...
                        {renderMeetingsList(upcomingMeetings, "No upcoming meetings scheduled.")}
                    </div>
  ...
  <div 
    ...
    style={{ 
      backgroundImage: "url('https://ik.imagekit.io/69rzkdyiaw/guh7.png')",
  ...
  <div className="relative z-20 px-6 py-8 flex justify-between items-center">
    <div>
      ...
      <p className="text-blue-100 text-sm">Manage your scheduled meetings</p>
    </div>
