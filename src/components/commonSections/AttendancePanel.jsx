...
const AttendancePanel = () => {
    ...
    const userId = "265116e9-0546-425b-bc9b-3cf69a7e47fb";
    ...
    const fetchStatus = async () => {
        setLoading(true);
        try {
            const res = await axios.post("https://wemeet-backend-latest.onrender.com/api/attendance/status", { user_id: userId });
    ...
    const handleCheckIn = async () => {
        ...
        try {
            await axios.post("https://wemeet-backend-latest.onrender.com/api/check-in", {
                user_id: userId,
    ...
    const handleCheckOut = async () => {
        setActionLoading(true);
        try {
            await axios.post("https://wemeet-backend-latest.onrender.com/api/attendance/check-out", { 
                user_id: userId,
                report: workReport,
            });
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans">
            {/* Notification */}
            <div className={`fixed top-4 right-4 transition-all duration-300 z-50 ${showNotification ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
                <div className={`rounded-lg shadow-lg p-3 flex items-center ${message.includes("failed") || message.includes("error") ? "bg-red-50 border-l-4 border-red-500" : "bg-green-50 border-l-4 border-green-500"}`}>
                    <div className="mr-2">
                        {message.includes("failed") || message.includes("error") ?
                            <FaExclamationCircle className="text-red-500" size={20} /> :
                            <FaCheckCircle className="text-green-500" size={20} />}
                    </div>
                    <p className={`text-sm font-medium ${message.includes("failed") || message.includes("error") ? "text-red-700" : "text-green-700"}`}>{message}</p>
                </div>
            </div>
                ...
                <header className="bg-white shadow-sm rounded-lg sticky top-0 z-10 mb-4">
                    <div className="p-4 flex justify-between items-center">
                        ...
                        <div className="flex items-center text-gray-600">
                            ...
                            <span className="font-medium text-sm">{new Date().toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}</span>
                        </div>
                    </div>
                </header>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                            <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                                <div className="flex items-center mb-2">
                                    ...
                                    <p className="ml-2 text-sm font-medium text-gray-700">Status</p>
                                </div>
                                <div className="flex items-center">
                                    {status && !status.checkOut ? (
                                        ...
                                        <div className="flex items-center">
                                            ...
                                            <span className="font-semibold text-blue-600 text-sm">Completed</span>
                                        </div>
                                    ) : (
                                        <div className="flex items-center">
                                            <div className="w-2 h-2 bg-gray-300 rounded-full mr-2"></div>
                                            <span className="font-semibold text-gray-500 text-sm">Not Started</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                                <div className="flex items-center mb-2">
                                    ...
                                    <p className="ml-2 text-sm font-medium text-gray-700">Check-in</p>
                                </div>
                            ...
                            <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                                <div className="flex items-center mb-2">
                                    ...
                                    <p className="ml-2 text-sm font-medium text-gray-700">Check-out</p>
                                </div>
                                <div className="font-semibold text-gray-800 text-sm">
                                    {status && status.checkOut ? (
                                        new Date(status.checkOut).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                                    ) : (
                                        "Not checked out"
                                    )}
                                </div>
                            ...
                            <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                                <div className="flex items-center mb-2">
                                    ...
                                    <p className="ml-2 text-sm font-medium text-gray-700">Duration</p>
                                </div>
                                <div className="font-semibold text-gray-800 text-sm">
                                    {status && !status.checkOut ? (
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-100 mb-4 overflow-hidden">
                            ...
                            {!status?.checkIn && (
                                <div className="md:flex">
                                    <div className="md:w-1/3 p-6 flex flex-col items-center md:items-start">
                                        ...
                                        <p className="text-gray-500 text-sm text-center md:text-left mb-3">Start your workday by clicking check-in.</p>
                                    ...
                                    <div className="md:w-2/3 bg-gray-50 p-6 md:border-l border-gray-100 md:border-t-0 border-t">
                                        <h3 className="text-sm uppercase font-medium text-gray-500 mb-4">Today's Information</h3>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                            <div className="bg-white rounded-md p-4 border border-gray-100 shadow-sm">
                                                <div className="flex items-center mb-2">
                                                    ...
                                                    <div className="ml-2 font-medium text-sm text-gray-700">Working Hours</div>
                                                </div>
                                                <p className="text-gray-500 text-xs">Work hours are tracked automatically from check-in to check-out.</p>
                                            </div>
                                            
                                            <div className="bg-white rounded-md p-4 border border-gray-100 shadow-sm">
                                                <div className="flex items-center mb-2">
                                                    <div className="w-8 h-8 rounded-md bg-green-50 flex items-center justify-center">
                                                        <FaClipboardList size={14} className="text-green-600" />
                                                    </div>
                                                    <div className="ml-2 font-medium text-sm text-gray-700">Work Summary</div>
                                                </div>
                                                <p className="text-gray-500 text-xs">Submit a work report when checking out at the end of your day.</p>
                                            </div>
                                        </div>
                                        
                                        <div className="bg-indigo-50 rounded-md p-4">
                                            <div className="flex items-center mb-2">
                                                <FaInfoCircle size={14} className="text-indigo-600 mr-2" />
                                                <h4 className="font-medium text-sm text-indigo-800">Getting Started</h4>
                                            </div>
                                            <p className="text-indigo-700 text-xs mb-2">Check in to start your workday. Your attendance will be recorded automatically.</p>
                            ...
                            {status?.checkIn && !status?.checkOut && (
                                <div className="md:flex">
                                    <div className="md:w-1/3 p-6 flex flex-col items-center md:items-start">
                                        ...
                                        <div className="w-32 h-32 relative mb-4">
                                            ...
                                            <div className="absolute inset-0 flex items-center justify-center flex-col">
                                                ...
                                                <div className="text-blue-600 text-xs mt-1 font-medium">Hours Worked</div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="md:w-2/3 bg-gray-50 p-6 md:border-l border-gray-100 md:border-t-0 border-t">
                                        <h3 className="text-sm uppercase font-medium text-gray-500 mb-4">Session Information</h3>
                                        ...
                                        <div className="bg-white rounded-md p-4 border border-gray-100 shadow-sm mb-4">
                                            <h4 className="text-xs font-medium text-gray-500 uppercase mb-3">Current Session</h4>
                                            <ul className="space-y-2">
                                                <li className="flex items-center justify-between">
                                                    <span className="text-gray-600 text-xs">Elapsed Time</span>
                                                    <span className="font-semibold text-gray-800 text-sm">{formatDuration(elapsedTime)}</span>
                                                </li>
                                                <li className="flex items-center justify-between">
                                                    <span className="text-gray-600 text-xs">Today's Date</span>
                                                    <span className="font-semibold text-gray-800 text-sm">{new Date().toLocaleDateString()}</span>
                                                </li>
                                                <li className="flex items-center justify-between">
                                                    <span className="text-gray-600 text-xs">Location Status</span>
                                                    <span className="font-semibold text-green-600 text-sm">Verified</span>
                                                </li>
                                            </ul>
                                        </div>

                                        <div className="bg-indigo-50 rounded-md p-3 flex items-start">
                                            <FaInfoCircle className="text-indigo-600 mt-0.5 flex-shrink-0 mr-2" size={14} />
                                            <p className="text-xs text-indigo-700">
                                                Your active session is being tracked. When checking out, you'll be asked to submit a work report.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="md:flex">
                                    <div className="md:w-1/3 p-6 flex flex-col items-center md:items-start">
                                        ...
                                        <div className="bg-white rounded-lg p-6 border border-gray-100 shadow-sm text-center w-full mb-4">
                                            ...
                                            <div className="text-xs text-gray-500 mb-1 uppercase tracking-wide font-medium">Total Work Duration</div>
                                    ...
                                    <div className="md:w-2/3 bg-gray-50 p-6 md:border-l border-gray-100 md:border-t-0 border-t">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                            ...
                                            <div className="bg-white rounded-md p-4 border border-gray-100 shadow-sm">
                                                <div className="flex items-center">
                                                    <div className="w-8 h-8 rounded-md bg-green-50 flex items-center justify-center">
                                                        <FaSignOutAlt size={14} className="text-green-600" />
                                                    </div>
                                                    <div className="ml-2">
                                        ...
                                        <div className="bg-white rounded-md p-4 border border-gray-100 shadow-sm mb-4">
                                            <div className="flex items-center justify-between mb-3">
                                                <h4 className="text-xs font-medium text-gray-500 uppercase">Work Summary</h4>
                                                <span className="text-xs px-2 py-0.5 bg-gray-100 rounded-full text-gray-600">{new Date().toLocaleDateString()}</span>
                                            </div>
                                            <div className="flex items-center mb-2">
                                                <FaClipboardList size={14} className="text-gray-400 mr-1" />
                                                <span className="font-medium text-gray-700 text-xs">Work Report</span>
                                            </div>
                                            <div className="bg-gray-50 p-3 rounded-md">
                                                <p className="text-gray-700 text-xs">{status.report || "No report submitted."}</p>
                                            </div>
                                        </div>

                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="bg-white rounded-md p-3 border border-gray-100 shadow-sm">
                                                <div className="text-xs font-medium text-gray-500 mb-1">Check-in Date</div>
                                                <div className="font-semibold text-gray-800 text-sm">{new Date(status.checkIn).toLocaleDateString()}</div>
                                            </div>
                                            <div className="bg-white rounded-md p-3 border border-gray-100 shadow-sm">
                                                <div className="text-xs font-medium text-gray-500 mb-1">Total Hours</div>
                                                <div className="font-semibold text-gray-800 text-sm">{getWorkDuration()}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                                <div className="flex items-center mb-2">
                                    ...
                                    <h3 className="ml-2 font-medium text-gray-700 text-sm">Working Hours</h3>
                                </div>
                                <p className="text-gray-600 text-xs">Your hours are tracked automatically from check-in to check-out. Remember to submit a report before checking out.</p>
                            </div>
                            ...
                            <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                                <div className="flex items-center mb-2">
                                    ...
                                    <h3 className="ml-2 font-medium text-gray-700 text-sm">Location Tracking</h3>
                                </div>
                                <p className="text-gray-600 text-xs">Your location is only captured at check-in to verify your presence. No continuous tracking is performed.</p>
                            </div>
                            ...
                            <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                                <div className="flex items-center mb-2">
                                    ...
                                    <h3 className="ml-2 font-medium text-gray-700 text-sm">Work Reports</h3>
                                </div>
                                <p className="text-gray-600 text-xs">Detailed summaries help track accomplishments. Be specific about tasks completed and goals achieved.</p>
                            </div>
                        </div>
            ...
            {showReportModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
                        <div className="mb-4">
                            <h2 className="text-lg font-bold text-gray-800 mb-1">Work Report</h2>
                            <p className="text-gray-500 text-sm">Summarize your accomplishments for today</p>
                        </div>
                        <div className="mb-4">
                            <textarea
