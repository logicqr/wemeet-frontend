...
export default function AdminLeaveRequests() {
  ...
  const fetchLeaveRequests = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://wemeet-backend-latest.onrender.com/api/admin-leave-request');
      if (response.data && response.data.adminLeaveRequests) {
        setLeaveRequests(response.data.adminLeaveRequests || []);
      } else {
        setLeaveRequests([]);
      }
      setError(null);
  ...
  const updateLeaveStatus = async (leaveId, status) => {
    ...
    try {
      await axios.post('https://wemeet-backend-latest.onrender.com/api/leave-request/update-status', {
        leave_id: leaveId,
        status: status,
        approvedBy: ADMIN_ID
      });
      ...
      setError(`Failed to ${status.toLowerCase()} the request. Please try again.`);
    } finally {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        <div className="mb-6 sm:mb-8">
          <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Leave Request Management</h1>
              ...
              <div className="relative w-full max-w-[200px]">
                <div className="relative">
                  <button
                    ...
                    onClick={() => document.getElementById('mobile-tabs').focus()}
                  >
          ...
          <div className="divide-y divide-gray-200">
            {loading ? (
              <div className="flex justify-center items-center py-16">
                ...
                <span className="ml-3 text-sm font-medium text-gray-500">Loading requests...</span>
              </div>
              ...
              filteredRequests.map((request) => (
                <div key={request.leave_id} className="p-4 sm:p-6 hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="sm:ml-4 flex-1">
                        <div className="flex items-center justify-between sm:justify-start">
                          <div>
                            <h3 className="text-sm font-medium text-gray-900">{request.user?.userName || 'Employee'}</h3>
                            <span className="text-xs text-gray-500">{request.user?.position || ''}</span>
                          </div>
                        ...
                        <div className="mt-2 grid grid-cols-1 xs:grid-cols-2 gap-2 text-xs text-gray-500">
                          <div className="flex items-center">
                            ...
                            <span className="hidden sm:inline">{formatDate(request.startDate)} — {formatDate(request.endDate)}</span>
                            <span className="sm:hidden">{formatDateShort(request.startDate)} — {formatDateShort(request.endDate)}</span>
                          </div>
                          <div className="flex items-center">
                            <svg className="flex-shrink-0 mr-1.5 h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                          ...
                          <div className="flex items-center xs:col-span-2 sm:col-span-1">
                            ...
                            <span className="hidden sm:inline">Requested on {formatDate(request.createdAt)}</span>
                            <span className="sm:hidden">Requested {formatDateShort(request.createdAt)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
